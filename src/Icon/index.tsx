...
const Icon = (props: IconPropsType) => {
    const { fill = '', name = '', size = '', className = '', style = {}, spin, ...other} = props

    const cls = classnames(prefixCls, `${prefixCls}-${name}`, className);
    const svgCls = classnames({ [`${prefixCls}-spin`]: name === 'loading' ? true : spin });
    ...
    function generate(data:any, props:IconPropsType) {
        return data.map((item:any, index:number) => {
            switch (item.name) {
                ...
                case 'polyline':
                    return <polyline {...props} {...item.attr} key={`polyline-${index}`} />
                case 'line':
